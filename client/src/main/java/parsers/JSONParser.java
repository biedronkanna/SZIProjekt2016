...
public class JSONParser {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				for (int j = 0; j < column.length(); j++) {
					...
					if (!object.isNull("forkLift")) {
						...
						FieldContainer.addField(object.getInt("id"), object.getInt("x"),
								object.getInt("y"), object.getBoolean("difficultToTraverse"),
								new Image(JSONParser.class.getClassLoader()
										.getResource("forklift.png").toString()),
								object.getBoolean("difficultToTraverse"));
						ForkLift.setOnMapDirection(
								object.getJSONObject("forkLift").getString("direction"));
					} else if (!object.isNull("storage")) {
						FieldContainer.addField(object.getInt("id"), object.getInt("x"),
								object.getInt("y"), object.getBoolean("difficultToTraverse"),
								new Image(JSONParser.class.getClassLoader()
										.getResource("foodstorage.png").toString()),
								object.getBoolean("difficultToTraverse"));
					} else {
						Image image;
						if (object.getBoolean("difficultToTraverse") == true) {
							image = new Image(JSONParser.class.getClassLoader()
									.getResource("fielddifficult.png").toString());
						} else {
							image = new Image(JSONParser.class.getClassLoader()
									.getResource("field.png").toString());
						}
						FieldContainer.addField(object.getInt("id"), object.getInt("x"),
								object.getInt("y"), object.getBoolean("difficultToTraverse"), image,
								object.getBoolean("difficultToTraverse"));
					}
	...
	public static void parseJSONPath(String url)
		...
		try {
			...
			for (int i = 0; i < pathList.length; i++) {
				if (pathList[i].equals("\"MOVE\"")) {
					...
					FieldContainer.addField(ForkLift.currentPosition, currentField.getX(),
							currentField.getY(),
							currentField.getIsdifficultToTraverse(), new Image(JSONParser.class
									.getClassLoader().getResource("forklift.png").toString()),
							currentField.getIsdifficultToTraverse());
