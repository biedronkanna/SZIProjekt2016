...
public class MainController implements Initializable {
	...
	public void resetButtonClicked() throws NullPointerException {
		resetButton.setOnAction(e -> {
			try {
				...
				ForkLift.setImage(new Image(MainController.class.getClassLoader()
	...
	public void fillCanvas(Map<Integer, Field> mp) {
		...
		while (it.hasNext()) {
			...
			gc.drawImage(field.getImage(), field.getX() * 50,
					(mapCanvas.getHeight() - field.getY() * 50) - 50); // 500 -
																		// 490 -
																		// 10
			System.out
					.println("Drawing " + field.getX().toString() + " " + field.getY().toString());
		}
	}
}
